...
function delete_old(file) {
    ...
    var currentFileName = file.substring(dirName.length + 1);
    ...
    fs.readdir(dirName, function (err, files) {
        ...
        for (var i = 0, len = files.length; i < len; i++) {
            if (files[i].indexOf(currentFileName) >= 0 && currentFileName !== files[i])
                rotated_files.push(files[i]);
        }

        rotated_files = rotated_files.sort(function (a, b) {
            return Number(a.substring(currentFileName + 1)) > Number(b.substring(currentFileName + 1)) ? -1 : 1;
        });

        for (var i = rotated_files.length - 1; i >= 0; i--) {
            if (RETAIN > i + 1) return;
    ...
    setInterval(function () {
        ...
        pm2.list(function (err, apps) {
            ...
            apps.forEach(function (app) {
                ...
                if (dealedApp.indexOf(app.name) !== -1 || typeof(app.pm2_env.axm_options.isModule) !== 'undefined' && !ROTATE_MODULE) return;
                dealedApp.push(app.name);
